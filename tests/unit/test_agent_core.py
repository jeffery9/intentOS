...
class TestAgentContext:
    ...
    def test_context_with_session(self):
        ...
        context = AgentContext(
            user_id="user123", session_id="session456", variables={"key": "value"}
        )
    ...
    def test_context_to_dict(self):
        """上下文转换为字典"""
        context = AgentContext(user_id="user123", session_id="session456")
...
class TestAgentResult:
    ...
    def test_success_result(self):
        """成功结果"""
        result = AgentResult(success=True, message="操作成功", data={"key": "value"})
    ...
    def test_failure_result(self):
        """失败结果"""
        result = AgentResult(
            success=False, message="操作失败", error="ErrorType: Something went wrong"
        )
    ...
    def test_result_with_next_actions(self):
        """带后续动作的结果"""
        result = AgentResult(
            success=True, message="部分完成", next_actions=["step1", "step2", "step3"]
        )
...
class TestAgentBase:
    ...
    async def test_agent_execute(self):
        ...
        class MockAgent(Agent):
            async def execute(self, intent: str, context: AgentContext) -> AgentResult:
                return AgentResult(
                    success=True, message=f"Executed: {intent}", data={"intent": intent}
                )
    ...
    def test_agent_with_custom_config(self):
        ...
        config = AgentConfig(name="Test Agent", max_iterations=5, enable_mcp=False)
...
class TestAgentContextVariables:
    ...
    def test_context_variable_update(self):
        """上下文变量更新"""
        context = AgentContext(user_id="user1", variables={"count": 1})
...
class TestAgentResultEdgeCases:
    ...
    def test_empty_result(self):
        """空结果"""
        result = AgentResult(success=True, message="")
    ...
    def test_none_data_result(self):
        """None 数据结果"""
        result = AgentResult(success=True, message="No data", data=None)

        assert result.data is None

    def test_empty_artifacts(self):
        """空产物列表"""
        result = AgentResult(success=True, message="Done", artifacts=[])

        assert len(result.artifacts) == 0

    def test_empty_next_actions(self):
        """空后续动作列表"""
        result = AgentResult(success=True, message="Done", next_actions=[])
