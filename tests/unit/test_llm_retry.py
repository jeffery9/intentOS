...
class TestRetryConfig:
    ...
    def test_config_calculate_delay_rate_limit(self):
        """测试速率限制延迟计算"""
        config = RetryConfig(rate_limit_base_delay=5.0, rate_limit_max_delay=300.0, jitter=0.0)
    ...
    def test_config_custom_values(self):
        """测试自定义配置值"""
        config = RetryConfig(max_retries=10, base_delay=2.0, max_delay=120.0, timeout=60.0)
...
class TestRetryAttempt:
    ...
    def test_attempt_default_creation(self):
        ...
        attempt = RetryAttempt(
            attempt_number=1, backend_name="test_backend", start_time=datetime.now()
        )
    ...
    def test_attempt_duration_without_end_time(self):
        """测试无结束时间的持续时间"""
        attempt = RetryAttempt(attempt_number=1, backend_name="test", start_time=datetime.now())
    ...
    def test_attempt_duration_with_end_time(self):
        ...
        attempt = RetryAttempt(
            attempt_number=1, backend_name="test", start_time=start, end_time=end
        )
    ...
    def test_attempt_to_dict(self):
        """测试尝试转换为字典"""
        attempt = RetryAttempt(
            attempt_number=2, backend_name="openai", start_time=datetime.now(), success=True
        )
...
class TestRetryResult:
    ...
    def test_result_success(self):
        """测试成功结果"""
        result = RetryResult(
            success=True, response={"data": "success"}, total_attempts=2, backends_used=["openai"]
        )
    ...
    def test_result_failure(self):
        """测试失败结果"""
        result = RetryResult(success=False, error="All retries failed", total_attempts=3)
    ...
    def test_result_to_dict_success(self):
        """测试成功结果转换为字典"""
        result = RetryResult(success=True, response={"key": "value"}, total_attempts=1)
    ...
    def test_result_to_dict_failure(self):
        """测试失败结果转换为字典"""
        result = RetryResult(success=False, error="Failed after retries")
...
class TestLLMRetryIntegration:
    ...
    def test_config_and_result_integration(self):
        ...
        result = RetryResult(success=True, total_attempts=3)
