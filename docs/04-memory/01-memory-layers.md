...
> IntentOS 的记忆系统分为3 Layer：工作记忆、短期记忆、长期记忆，模拟人类记忆系统，每层有不同的存储策略和生命周期。
