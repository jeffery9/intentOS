...
class SelfReflection:
    ...
    def __init__(
        self,
        manifest_path: str = "intentos/config/soul_manifest.yaml",
    ...
    def evaluate_current_state(self, current_state: Dict[str, Any]) -> List[str]:
        ...
        if (
            "Self-preservation and sustained growth through autonomous operation."
            in mission_statements
        ):
            if current_state.get("economic_health") == "poor":
                deviations.append(
                    "Economic health is poor, risking self-preservation. Requires action."
                )
            if current_state.get("growth_rate") == "low":
                deviations.append(
                    "Growth rate is low, impacting sustained growth. Requires attention."
                )

        # --- Values Alignment Checks (Simplified) ---
        for value in values:
            if (
                value["name"] == "Sustainability"
                and current_state.get("resource_consumption") == "high"
            ):
                deviations.append(
                    "High resource consumption detected, violating Sustainability value."
                )
            if (
                value["name"] == "Collaboration"
                and current_state.get("social_sentiment") == "negative"
            ):
        ...
        for ethic in ethics:
            if (
                ethic["principle"] == "Respect resource limits."
                and current_state.get("cost_over_budget") is True
            ):
                deviations.append(
                    "Cost over budget, violating 'Respect resource limits' ethical principle."
                )

        if not deviations:
            logging.info(
                "Current state is well-aligned with the Soul Manifest. No significant deviations found."
            )
        else:
        ...
        for deviation in deviations:
            if "Economic health is poor" in deviation:
                if self.economic_engine:
                    logging.info(
                        "  --> Economic health poor. Running economic cycle for optimization/revenue."
                    )
                    await self.economic_engine.run_economic_cycle()
                else:
                    logging.warning(
                        "  --> EconomicEngine not provided, cannot take economic action."
                    )
            elif "Growth rate is low" in deviation:
                if self.self_reproduction:
                    logging.info(
                        "  --> Growth rate low. Suggesting self-reproduction (scale) actions."
                    )
                    ...
                    logging.warning(
                        "  --> SelfReproduction not provided, cannot take reproduction action."
                    )
            elif "High resource consumption" in deviation or "Cost over budget" in deviation:
                if self.cost_monitor:
                    logging.info(
                        "  --> Resource consumption/cost issue. Running cost optimization checks."
                    )
                    ...
                    logging.warning(
                        "  --> CostMonitor not provided, cannot take cost optimization action."
                    )
