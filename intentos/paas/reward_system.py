...
class RewardSystem:
    ...
    def award_credits(self, entity_id: str, contribution_type: str, quantity: float = 1.0) -> float:
        ...
        logging.info(
            f"Awarded {credits_to_award} credits to {entity_id} for {contribution_type}. New balance: {self.ledger[entity_id]}"
        )
    ...
    def spend_credits(self, entity_id: str, amount: float) -> float:
        ...
        logging.info(
            f"Entity {entity_id} spent {amount} credits. New balance: {self.ledger[entity_id]}"
        )
        return self.ledger[entity_id]
    ...
    def process_gas_settlement(
        ...
        logging.info(
            f"Gas Settlement: App {app_id} used {gas_used} gas. User {user_id} paid {total_cost:.4f}. Developer {developer_id} earned {developer_share:.4f}."
        )
