...
class IntentGraph:
    ...
    def add_edge(self, edge: IntentEdge) -> None:
        ...
        if edge.source_id not in self.nodes or edge.target_id not in self.nodes:
            raise ValueError("边的源节点或目标节点不存在")
    ...
    def get_neighbors(
        self, node_id: str, edge_type: Optional[IntentEdgeType] = None
    ) -> list[IntentNode]:
        """获取邻居节点"""
    ...
    def search_by_keyword(self, keyword: str) -> list[IntentNode]:
        ...
        for node in self.nodes.values():
            if (
                keyword_lower in node.name.lower()
                or keyword_lower in node.description.lower()
                or any(keyword_lower in tag.lower() for tag in node.tags)
            ):
                results.append(node)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_degree": sum(len(neighbors) for neighbors in self.adjacency.values())
