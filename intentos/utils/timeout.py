...
class TimeoutError(Exception):  # type: ignore  # noqa: A004
    """超时错误"""
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    delay = min(
                        self.config.retry_base_delay * (2**attempt), self.config.retry_max_delay
                    )
                    logger.warning(
                    ...
                    logger.error(
                        f"{operation_name} 超时，已达最大重试次数 " f"({self.config.max_retries}次)"
                    )
