...
class GlobalCoordinator:
    ...
    def _select_best_region(
        ...
        if user_location:
            ...
            if active_regions:
                best_region = min(
                    active_regions,
                    key=lambda r: self._calculate_distance(user_location, r.location),
                )
                return best_region
    ...
    async def _update_global_state(self) -> None:
        ...
        self.global_state.average_load = total_load / len(self.regions) if self.regions else 0
...
class RegionCoordinator:
    ...
    async def get_state(self) -> dict:
        """获取区域状态"""
        node_states = [await agent.get_state() for agent in self.nodes.values()]
