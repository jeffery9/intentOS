...
class ClusterMembership:
    ...
    def can_join(self) -> bool:
        ...
        return self.cluster_id is None or self.status == MembershipStatus.SUSPENDED
...
class VMCommunity:
    ...
    def get_active_members(self) -> list[str]:
        """获取活跃成员 ID 列表"""
        return [node_id for node_id, membership in self.members.items() if membership.is_member()]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "metadata": self.metadata,
            "members": {node_id: m.to_dict() for node_id, m in self.members.items()},
        }
    ...
    def from_dict(cls, data: dict) -> VMCommunity:
        ...
        community.members = {
            node_id: ClusterMembership.from_dict(m_data) for node_id, m_data in members_data.items()
        }
        return community
...
class ConsensusNotReachedError(Exception):
    ...
    def __init__(self, community_id: str, votes: int, required: int):
        ...
        super().__init__(f"社区 {community_id} 共识未达成：{votes}/{required} 票")
