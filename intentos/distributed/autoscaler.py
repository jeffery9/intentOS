...
class AutoScaler:
    ...
    def decide_scaling(
        ...
        if metrics.cpu_utilization > self.policy.target_cpu_utilization:
            ...
            target_replicas = max(
                self.policy.min_replicas, int(self.current_replicas * scale_factor)
            )
        ...
        # 应用扩缩容速率限制
        if direction == ScaleDirection.SCALE_UP:
            max_increase = int(self.current_replicas * self.policy.scale_up_rate_percent / 100)
            target_replicas = min(
                target_replicas, self.current_replicas + max_increase, self.policy.max_replicas
            )

        elif direction == ScaleDirection.SCALE_DOWN:
            max_decrease = int(self.current_replicas * self.policy.scale_down_rate_percent / 100)
            target_replicas = max(
                target_replicas, self.current_replicas - max_decrease, self.policy.min_replicas
            )
        ...
        if direction == ScaleDirection.SCALE_UP and self._scale_up_cooldown:
            if (
                now - self._scale_up_cooldown
            ).total_seconds() < self.policy.scale_up_stabilization_seconds:
                direction = ScaleDirection.NO_CHANGE

        elif direction == ScaleDirection.SCALE_DOWN and self._scale_down_cooldown:
            if (
                now - self._scale_down_cooldown
            ).total_seconds() < self.policy.scale_down_stabilization_seconds:
                direction = ScaleDirection.NO_CHANGE
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "scale_up_cooldown": self._scale_up_cooldown.isoformat()
            if self._scale_up_cooldown
            else None,
            "scale_down_cooldown": self._scale_down_cooldown.isoformat()
            if self._scale_down_cooldown
            else None,
        }
