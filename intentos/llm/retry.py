    ...
    async def execute(
        ...
        **kwargs,
        ...
        for backend in backends_to_try:
            backend_result = await self._execute_with_retries(
                backend, messages, **kwargs
            )
        ...
        if not result.success:
            errors = [
                f"[{a.backend_name}] {a.error}"
                for a in result.attempts if a.error
            ]
            result.error = "所有重试失败:\n" + "\n".join(errors)
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                ...
                response = await asyncio.wait_for(
                    self._call_backend(backend, messages, **kwargs),
            ...
            if attempt < self.config.max_retries:
                is_rate_limit = (
                    attempt_record.error_type == RetryableErrorType.RATE_LIMIT
                )
